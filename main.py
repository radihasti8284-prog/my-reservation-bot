...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        default = [(0, '09:00', '18:00', 2, 1), (1, '09:00', '18:00', 2, 1), (2, '09:00', '18:00', 2, 1),
                   (3, '09:00', '18:00', 2, 1), (4, '09:00', '18:00', 2, 1), (5, '09:00', '14:00', 1, 0),
                   (6, '09:00', '14:00', 0, 0)]
        cursor.executemany(
...
def update_support_contact(val):
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("UPDATE settings SET value = ?, updated_at = CURRENT_TIMESTAMP WHERE key = 'support_contact'",
                   (val,))
    conn.commit()
    conn.close()
...
def increment_capacity(date, time_slot):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO daily_capacity (appointment_date, appointment_time, booked_count, max_capacity) VALUES (?, ?, 1, 2) ON CONFLICT(appointment_date, appointment_time) DO UPDATE SET booked_count = booked_count + 1",
        (date, time_slot))
    conn.commit()
    conn.close()
...
def decrement_capacity(date, time_slot):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE daily_capacity SET booked_count = booked_count - 1 WHERE appointment_date = ? AND appointment_time = ? AND booked_count > 0",
        (date, time_slot))
    conn.commit()
    conn.close()
        ...
        if 'message' in data:
            ...
            if text == '/start':
                send_message(chat_id,
                             ...
                             "👇 برای رزرو کلیک کنید:",
                             reply_markup={
                                 "inline_keyboard": [[{"text": "📅 رزرو نوبت", "web_app": {
                                     "url": "https://my-reservation-bot.onrender.com/static/landing.html"}}]
                             }
                             )
                elif text == '/admin':
                conn = get_db()
                ...
                if row and row['is_admin'] == 1:
                    send_message(chat_id, "👋 پنل ادمین", reply_markup={"inline_keyboard": [[{"text": "📊 مدیریت",
                                                                                             "web_app": {
...
def create_appointment():
    ...
    cursor.execute(
        "INSERT INTO appointments (user_id, service_id, appointment_date, appointment_time, status, receipt) VALUES (?, ?, ?, ?, 'pending', ?)",
...
def get_user_appointments(telegram_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        '''SELECT a.*, s.name as service_name FROM appointments a JOIN users u ON a.user_id = u.id JOIN services s ON a.service_id = s.id WHERE u.telegram_id = ? ORDER BY a.appointment_date DESC''',
        (telegram_id,))
    apps = cursor.fetchall()
    conn.close()
...
def update_appointment(appointment_id):
    ...
    cursor.execute(
        "UPDATE appointments SET appointment_date = ?, appointment_time = ? WHERE id = ? AND status = 'pending'",
...
def admin_update_status(appointment_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        '''SELECT a.user_id, u.telegram_id, a.appointment_date, a.appointment_time, s.name as service_name FROM appointments a JOIN users u ON a.user_id = u.id JOIN services s ON a.service_id = s.id WHERE a.id = ?''',
        (appointment_id,))
    app = cursor.fetchone()
...
def update_schedule():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE work_schedule SET start_time = ?, end_time = ?, capacity = ?, is_active = ? WHERE day_of_week = ?",
...
def export_excel():
    ...
    cursor = conn.cursor()
    cursor.execute(
        '''SELECT a.*, u.name as user_name, u.phone, s.name as service_name FROM appointments a JOIN users u ON a.user_id = u.id JOIN services s ON a.service_id = s.id ORDER BY a.appointment_date DESC''')
    apps = cursor.fetchall()
    conn.close()
    ...
    return send_file(output, as_attachment=True, download_name=f"nobat-ha-{datetime.now().strftime('%Y%m%d')}.xlsx",
