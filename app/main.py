...
def init_db():
    ...
    # جدول services (فقط یک خدمت: کوتاهی مو)
    cursor.execute('''
    ...
    # ====== حذف خدمات قبلی و اضافه کردن فقط کوتاهی مو ======
    cursor.execute("DELETE FROM services")  # حذف همه خدمات قبلی

    # فقط یک خدمت: کوتاهی مو با قیمت ۲۰۰,۰۰۰ تومان
    services = [
    ...
    conn.commit()
    conn.close()
    print("✅ دیتابیس راه‌اندازی شد (فقط خدمت کوتاهی مو با بیعانه ۲۰۰,۰۰۰ تومان)")
...
def admin_add_service():
    # برای جلوگیری از اضافه شدن خدمات جدید توسط ادمین
